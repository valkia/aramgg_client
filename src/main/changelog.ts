...
export const LOCAL_CHANGELOG_ENTRIES: ChangelogEntry[] = [
  {
    version: '0.2.17',
    date: '2026-09-09',
    title: '修复海克斯推荐与悬浮窗无法显示',
    summary: '修复 0.2.16 更新后窗口启动超时、海克斯推荐和模拟浮窗无法显示的问题。',
    changes: [
      '修复窗口就绪通知缺失导致的 30 秒启动超时，恢复后续游戏流程监控初始化。',
      '补齐英雄监控状态读取接口，恢复启动时的英雄状态显示。',
      '为窗口通信接口增加类型检查与回归测试，防止同类接口遗漏。',
    ],
  },
  {
    version: '0.2.16',
    date: '2026-09-07',
    title: '英雄监控与浮窗稳定性优化',
    summary: '减少英雄监控的重复处理，保持窗口预加载，并修复窗口准备期间或停止识别后的旧结果干扰。',
    changes: [
      '英雄监控复用后台状态，减少重复查询与缓存写入，停止和恢复监控时保持状态一致。',
      '英雄详情和海克斯浮窗继续提前加载，等待页面准备完成后再发送首条数据。',
      '修复停止识别或游戏阶段变化后，延迟返回的旧结果重新弹出窗口的问题。',
      '完善窗口关闭后的重新创建和加载失败重试，减少首次展示异常。',
    ],
  },
  {
    version: '0.2.15',
    date: '2026-08-31',
    title: '战绩查询、问题反馈与运行优化',
    summary: '新增海克斯大乱斗战绩查询和应用内反馈入口，补充英雄详情显示控制，并减少识别失败后的重复处理。',
    changes: [
      '新增当前账号的海克斯大乱斗战绩查询，支持翻页查看胜负、英雄、KDA、对局时间与时长。',
      '战绩列表展示每局实际使用的海克斯和装备，并补充名称与图标，方便回顾对局。',
      '主窗口新增意见反馈入口，支持填写问题、附加截图和联系方式，提交时自动附带脱敏后的近两天日志。',
      '新增“展示英雄详情”开关，可控制英雄详情自动弹出，同时保留英雄监控与后台功能。',
      '修复海克斯识别失败后反复执行完整识别的问题，减少无效截图与识别开销。',
    ],
  },
  {
    version: '0.2.14',
    date: '2026-08-27',
    title: '战绩贡献版本校验',
    summary: '匿名战绩贡献按当前游戏补丁筛选，清理旧补丁的待上传记录，减少无效上传。',
    changes: [
      '按服务端指定的当前游戏补丁筛选待上传战绩，避免旧补丁数据混入当前版本统计。',
      '自动移除上传队列中不符合目标补丁的记录，保留本地战绩缓存。',
      '完善上传前的版本校验，未取得有效目标补丁时暂停上传。',
    ],
  },
  {
    version: '0.2.13',
    date: '2026-08-24',
    title: '恢复官方排名推荐与胜率提示',
    summary: '海克斯推荐重新优先采用官方排名和梯队，保留胜率参考，并在缺少胜率时显示“场次不足”。',
    changes: [
      '英雄详情海克斯列表与浮窗优先推荐统一采用官方排名规则，缺少完整排名时优先参考梯队和选取率。',
      '保留有效胜率数据作为参考；即使胜率暂缺，也能利用已有排名和梯队展示推荐。',
      '英雄详情和海克斯浮窗在胜率缺失时显示“场次不足”，并同步提供英文与繁体中文提示。',
    ],
  },
  {
    version: '0.2.12',
    date: '2026-08-10',
    title: '战绩贡献上传优化',
    summary: '匿名战绩贡献切换至新的上传服务，压缩上传数据并完善重试处理，线上上传仅在官方安装包中启用。',
    changes: [
      '匿名战绩贡献切换至 ARAMGG 主站上传服务，上传数据先压缩，减少网络传输量。',
      '完善战绩归档状态的重试处理，避免暂时无法接收的战绩被永久标记为上传失败。',
      '线上战绩上传仅在官方发布的安装包中启用，本地开发和普通自行打包版本默认不向线上上传。',
    ],
  },
  {
    version: '0.2.11',
